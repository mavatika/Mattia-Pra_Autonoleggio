...
if (count($_POST) > 0) {
  ...
  if (count($params) == 0) {
    try {
      ...
      if (empty($carID)) throw new Exception('CAR ID EMPTY');
      ...
      if (strpos($e->getMessage(), 'Failed to parse time') !== false) $e = 'La data inserita è in un formato non valido, deve essere nel formato MM/GG/AAAA';
      $errs .= '<p class="server_message error_el">'.$e.'</p>'."\n"; 
    }
  } else {
